...
<table class="layui-table" lay-data="{width: 1300, height:500, url:'<%=basePath%>/user/findAll?roleId=${roleId}', page:true, id:'idTest'}" lay-filter="demo">
  <thead>
    <tr>
     <!--  <th lay-data="{type:'checkbox', fixed: 'left'}"></th> -->
      <th lay-data="{field:'id', width:60, sort: true, fixed: true}">ID</th>
      <th lay-data="{field:'username', width:500}">用户名</th>
		...
		$.ajax({
	        ...
	        success: function (data) {
				if(data.result==true){
	               ...
	               setTimeout(function () {
	                   window.location.href = "<%=basePath%>user/page?roleId=${roleId}";
	               }, 1000);
